...
class DecayingSubstrate(JAXSubstrate):
    ...
    The frustration between constraint pair (i, j) decays as:
        ε_ij(t) = ε_ij(0) · exp(-Δt / τ_ij)
    where:
        τ_ij = tau_base / min(λ_i, λ_j)
        Δt   = number of decay_step() calls since last ping on (i, j)

    Decayed edges (ε_ij < epsilon_floor) are removed from the *active*
    frustration graph.  The constraints themselves stay registered.

    Overrides _min_nonzero_frustration() and _average_degree() to draw
    from the decay cache, so MPCCluster.separation_bound() automatically
    uses the decayed graph — making N_max grow as edges decay.

    New methods (AMEND-001):
        decay_step()             — advance one time step
        ping(i, j, strength)    — re-stamp an edge toward original value
        update_frustration(i,j) — AMEND-004 direct setter
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Sorted pid-pair keys: (min_pid, max_pid)
        self._decay_cache: Dict[Tuple[str, str], float] = {}
        self._initial_eps: Dict[Tuple[str, str], float] = {}
        # uid → proposition_id (needed to bridge uid-keyed frustration dict)
    ...
    def frustration(self, v: np.ndarray) -> Dict:
        """
        Compute fresh pairwise frustration (parent implementation), then
        initialise the decay cache for any *newly-seen* pairs.

        IMPORTANT: existing entries are NOT overwritten — their decayed
        values are preserved across calls.
        """
    ...
    def decay_step(self):
        ...
        Edges that fall below epsilon_floor are removed from the active
        ...
        for key in list(self._active_pairs):
            ...
            self._decay_cache[key] = self._decay_cache.get(key, 0.0) * np.exp(-1.0 / tau)
    ...
    def update_frustration(self, i: str, j: str, new_epsilon: float):
        """
        AMEND-004 interface: directly set ε_ij to new_epsilon.
        Activates or deactivates the edge as appropriate.
        """
        key = (min(i, j), max(i, j))
